...
class ModuleControl(object):
    ...
    def findmeta(self, address):
        ...
        meta = self.df[filt]
        #self.testing = [[x for x in x] for x in meta.values]
        #self.testing = self.testing[0]
    ...
    def viewsensingdata(self):
        #display.lcd_long_write(display, str(self.modulenest), 1)
        for key in self.modulenest.keys():
            if self.modulenest[key][2] == 'sensor' :
                display.lcd_clear()
                self.bus.write_byte(0x08, 0x02) # always 0x02
                self.currentData[key] = self.bus.read_byte(0x08)
                display.lcd_clear()
    ...
    def whattest(self, address):
        #for key in self.modulenest.keys():
        if self.modulenest[key][2] == 'sensor' :
            display.lcd_clear()
            self.bus.write_byte(address, 0x02) # always 0x02
            self.currentData[key] = self.bus.read_byte(address)
            display.lcd_clear()
            display.lcd_long_write(display, str(self.modulenest[key][1]) + ' :', 1)
            display.lcd_long_write(display, str(self.currentData[key]), 2)
            time.sleep(3)
